...
{
    public partial class DistWiseAbstract : System.Web.UI.Page
    {
        ...
        protected void BindSSRS()
        {
            try
            {
                ...
                ReportViewer1.ServerReport.ReportPath = "/SSYDistrictWiseReport";
                ...
                {
                    if (parameters[i].Name.ToLower() == "loccodes")
                    {
                        parmList.Add(new ReportParameter(parameters[i].Name.ToString(), Request.QueryString["sendloactions"].ToString(), false));
                    }
...
//parmList.Add(new ReportParameter(parameters[i].Name.ToString(), Request.QueryString["todate"].ToString(), false));
                    ...
                    if (parameters[i].Name.ToLower() == "distnames")
                    {
                        parmList.Add(new ReportParameter(parameters[i].Name.ToString(), Request.QueryString["disid"].ToString(), false));
                    }
                    if (parameters[i].Name.ToLower() == "subdivname")
                    {
                        parmList.Add(new ReportParameter(parameters[i].Name.ToString(), Request.QueryString["subdivid"].ToString(), false));
                    }
                    if (parameters[i].Name.ToLower() == "locnames")
                    {
