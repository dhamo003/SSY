...
{
    public partial class LocWiseProffDetCount : System.Web.UI.Page
    {
        ...
        protected void BindSSRS()
        {
            try
            {
                ...
                ReportViewer1.ServerReport.ReportPath = "/SSY_ProfessionalSummary";
                ...
                {
                    if (parameters[i].Name.ToLower() == "locationid")
                    {
                        parmList.Add(new ReportParameter(parameters[i].Name.ToString(), Request.QueryString["Ids"].ToString(), false));
                    }
                    ...
                    {
                        parmList.Add(new ReportParameter(parameters[i].Name.ToString(), DateTime.ParseExact(Request.QueryString["frDate"].ToString(), "dd-MM-yyyy", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd"), false));
                    }
                    if (parameters[i].Name.ToLower() == "indtype")
                    {
                        parmList.Add(new ReportParameter(parameters[i].Name.ToString(), Request.QueryString["IndType"].ToString(), false));
                    }
                     if (parameters[i].Name.ToLower() == "gender")
                    {
                        parmList.Add(new ReportParameter(parameters[i].Name.ToString(), Request.QueryString["Gender"].ToString(), false));
                    }
